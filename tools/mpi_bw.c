 ...
 *
 * usage: mpi_bw [bytes] [window] [rounds] [flip]
 *   flip=0  the host holding rank 0 sends
 *   flip=1  the other host sends
 *
...
{
	int rank, size, i, w, peer = -1, sender = 0, pairs = 0, flip = 0;
	size_t bytes = 8u << 20;
	...
	req = malloc(sizeof(*req) * (size_t)window);
	if (!buf || !req) MPI_Abort(MPI_COMM_WORLD, 1);
	memset(buf, rank + 1, bytes * (size_t)window);

	for (i = 0; i < 2; i++) {          /* warm-up */
		for (w = 0; w < window; w++) {
			if (sender) MPI_Isend(buf + (size_t)w * bytes, (int)bytes, MPI_BYTE, peer, 0, MPI_COMM_WORLD, &req[w]);
			else        MPI_Irecv(buf + (size_t)w * bytes, (int)bytes, MPI_BYTE, peer, 0, MPI_COMM_WORLD, &req[w]);
		}
		MPI_Waitall(window, req, MPI_STATUSES_IGNORE);
	}
	...
	if (!rank) {
		double total = (double)bytes * window * rounds * pairs;
		double per_pair = (double)bytes * window * rounds;
	...
	free(buf); free(req); free(all); free(lista); free(listb);
	MPI_Finalize();
	return 0;
}
